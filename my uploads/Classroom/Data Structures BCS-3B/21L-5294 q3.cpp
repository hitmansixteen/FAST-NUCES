...
public:
	...
	bool pop(int& x) {
		...
		swap(arr[bottom], arr[mod((bottom + 1), maxsize)]);
		x = arr[bottom];
		bottom = mod(++bottom, maxsize);// to remove the second pushed element

		/*x = arr[top];
		top = mod(--top,maxsize);*/ // to remove according to LIFO property
