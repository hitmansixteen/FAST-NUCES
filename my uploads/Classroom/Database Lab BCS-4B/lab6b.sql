...
create view q3 as
	select o.CustomerNo,sum(q1.totalprice) as totalprice
...
create procedure placeOrder 
	...
	begin
		...
		select @quantityInStore = i.[Quantity in Store]
		from Items i
		where i.ItemNo = @itemNumber
		...
		insert into OrderDetails values (@orderNumber,@itemNumber,@quantity)
...
exec placeOrder @orderNumber = 4, @itemNumber = 400, @quantity = 150
...
create procedure cancelOrder
	@customerNumber varchar(2),
	@orderNumber int
	as
	begin

		if exists(select * from [Order] o where o.CustomerNo = @customerNumber and o.OrderNo = @orderNumber)
		begin
			delete from [OrderDetails]
			where OrderNo = @orderNumber

			delete from [Order]
			where OrderNo = @orderNumber and CustomerNo = @customerNumber
			
		end
		else
		begin
			print 'Order no '+ cast(@orderNumber as varchar) + ' is not of ' + @customerNumber
		end
...
exec cancelOrder @customerNumber = 'C1',@orderNumber = 2

--q4--------------------------

create procedure customerPoints
	@customerName varchar(30),
	@totalPoints int output
	as
	begin 
		select @totalPoints = sum(od.Quantity*i.Price)/100
		from Customers c join [Order] o on c.CustomerNo = o.CustomerNo join OrderDetails od on o.OrderNo = od.OrderNo join Items i on i.ItemNo = od.ItemNo
		where c.Name = @customerName
		group by c.[Name]
	end
go

drop procedure customerPoints

select * from [Order]
select * from Customers

declare @outPoints float
exec customerPoints @customerName = 'BILAL', @totalPoints = @outPoints output
