...
--1
select f1.FollowedUserName,count(*) "Number of followers"
from Following f1
...
--2 
select top 1 f.FollowedUserName as UserName ,count(f.FollowedUserName) as NoOfFollowers
from Following f
...
--4 
select  h.Hashtag ,t.UserName,count(t.UserName) as noOfTimes
 from Hashtags h,Tweets t
 where t.Text like ('%' + h.Hashtag + '%')
...
where not exists (
	select * 
	from Tweets t
	where u.UserName = t.UserName and t.text like '%Census%'
)
...
--8
select ut.InterestID,count(*) NumberOfUsers
from UserInterests ut
group by ut.InterestID
having count(*) >=All (
	select count(*)
	from UserInterests
	group by InterestID
)
select ut.InterestID,count(*) 'Number of users'
from UserInterests ut
group by ut.InterestID
having count(*) <=All (
	select count(*)
...
--9
select u.Country ,count(*) 'Tweets per country'
...
SELECT UserName
FROM (
    SELECT UserName, COUNT(*) AS NumTweets, AVG(COUNT(*)) OVER () AS AvgNumTweets
    FROM Tweets
    GROUP BY UserName
) AS T
WHERE NumTweets > AvgNumTweets
